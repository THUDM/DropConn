...
def train_vanilla(model, device, loader, optimizer, task_type, order):
    total_loss = 0
    model.train()
...
def train(model, device, loader, optimizer, task_type, args):
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            forward = lambda perturb: model(args.order, batch, perturb).to(torch.float32)[is_labeled]
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, args.epochs + 1):
            if args.flag:
                ...
                loss = train_vanilla(model, device, train_loader, optimizer, dataset.task_type, args.order)
