    ...
    for run in range(args.runs):
        ...
        if args.gnn == 'gin':
            model = GNN(gnn_type='gin', num_tasks=dataset.num_tasks, num_layer=args.num_layer, emb_dim=args.emb_dim,
                        ...
                        drop_ratio=args.drop_ratio, virtual_node=False, adj_learn=args.adj_learn).to(device)
        elif args.gnn == 'gin-virtual':
            model = GNN(gnn_type='gin', num_tasks=dataset.num_tasks, num_layer=args.num_layer, emb_dim=args.emb_dim,
                        edge_hidden_channels=args.edge_hidden_channels,
                        drop_ratio=args.drop_ratio, virtual_node=True, adj_learn=args.adj_learn).to(device)
        elif args.gnn == 'gcn':
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch > args.epochs // 100 and epoch % args.test_freq == 0 or epoch == args.epochs:
                ...
                print({'Epoch': epoch, 'Train': train_perf, 'Validation': valid_perf, 'Test': test_perf, 'Loss': loss,
                       'Num_batch': num_batch})
                result = (
