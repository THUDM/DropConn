...
class GNN_node(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, edge_hidden_channels, drop_ratio=0.5, JK="last", residual=False, gnn_type='gin', adj_learn=1):
        '''
            emb_dim (int): node embedding dimensionality
...
def get_adj_matrix(edge_index_fea, N, mode="train"):
    adj = torch.zeros([N, N])
    ...
    if mode == 'train':
        drop_rates = torch.FloatTensor(np.ones(N) * drop_rate)
        masks = torch.bernoulli(1. - drop_rates).unsqueeze(1)
        new_adj = masks.to(adj.device) * adj
        # print("mask new: ", new_adj)
    else:
        new_adj = adj * (1. - drop_rate)
    adj = new_adj
    adj = adj + torch.eye(N)
    # print("adj: ", adj)
...
# def get_adj_matrix(edge_index_fea, N, edge_weight_emb):
#     adj = torch.zeros([N, N], dtype=torch.float32).to(edge_index_fea.device)
#     adj[edge_index_fea[0, :], edge_index_fea[1, :]] = edge_weight_emb
#     adj = adj + torch.eye(N).to(edge_index_fea.device)
#     # non-symmetric D^(-1)A
#     degree = adj.sum(1)
#     r_inv = torch.pow(degree, -1)
    ...
    def forward(self, order, batched_data, perturb=None):
        ...
        for i in range(batched_data.y.shape[0]):
            ...
            if self.adj_learn:
                ...
                edge_attr_in_batch = self.bond_encoder(edge_attr_in_batch)
                edge_weight_emb = self.edge_linear(edge_attr_in_batch.float()).view(-1)
                edge_weight_emb = torch.sigmoid(edge_weight_emb).unsqueeze(1)
                # drop edge
                drop_rate = 0.2
                M = edge_weight_emb.size(0)
                if self.training:
                    drop_rates = torch.FloatTensor(np.ones(M) * drop_rate)
                    masks = torch.bernoulli(1. - drop_rates).unsqueeze(1)
                    edge_weight_emb = masks.to(edge_weight_emb.device) * edge_weight_emb
                else:
                    edge_weight_emb = edge_weight_emb * (1. - drop_rate)
                edge_weight_emb = edge_weight_emb.squeeze(dim=-1)
                ...
                if self.training:
                    ...
                    mode = "test"
                adj = get_adj_matrix(edge_index_fea, N, mode=mode).to(edge_index.device)
            # order = 1
            # print("new fea: ", new_fea.shape)
            # print("adj: ", adj.shape)
            new_fea = propagate(new_fea, adj, order)
            # print("new fea: ", new_fea.shape)
            # exit(1)
            new_fea_list.append(new_fea)
        tmp = torch.cat(new_fea_list, dim=0)
